...
bool comp(pair<vector <string>,int> a, pair<vector <string>,int> b){
...
void solve(){
    int n;
    cin >> n;
    vector <pair<vector <string>,int>> a(n);
    for (int i = 0; i < n; i++){
        int m;
        cin >> m;
        a[i].first.resize(m);
        if (m)
            getline(cin, a[i].first[0]);
        for (int j = 0; j < m; j++){
            getline(cin, a[i].first[j]);
            a[i].second += a[i].first[j].size();
        }
    }
